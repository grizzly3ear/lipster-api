...
class RepositoryGenerator extends Command
{
    ...
    protected function appendRoutes($name) {
        ...
        $route = $route."\n\tRoute::get('{{$lowerName}_id}', '{$name}Controller@get{$name}ById')->where('{$lowerName}', '[0-9]+');";
        $route = $route."\n\tRoute::post('', '{$name}Controller@create{$name}');";
        $route = $route."\n\tRoute::put('{{$lowerName}_id}', '{$name}Controller@update{$name}ById')->where('{$lowerName}', '[0-9]+');";
        $route = $route."\n\tRoute::delete('{{$lowerName}_id}', '{$name}Controller@delete{$name}ById')->where('{$lowerName}', '[0-9]+');";
        $route = $route."\n});\n";
